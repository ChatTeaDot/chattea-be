...
export class MatchingService {
  /**
   * MatchingService에서 사용할 MatchingRepository 의존성을 주입한다.
   *
   ...
   * @returns Black 후보 목록
   */
  async blackCandidates(userId: string, viewerPlanId: string): Promise<MatchCandidatePayload[]> {
   ...
   */
  async likedMeCandidates(userId: string, viewerPlanId: string, now = new Date()): Promise<MatchCandidatePayload[]> {
    validateUuid(userId);
   ...
   * @param planId 사용자 플랜 ID
   * @param now 기준 시각
   * @returns 매칭 결과
   */
  async likeUser(userId: string, likedUserId: string, planId = "free", now = new Date()): Promise<LikeUserPayload> {
    validateUuid(userId);
